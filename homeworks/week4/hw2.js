...
function listBooks(){
  request(`${apiUrl}/books?_limit=30`, function (err, response, body) {
    if(err){
...
function readBooks(id){
  request(`${apiUrl}/books/${id}`, function (err, response, body) {
    if(err){
      console.log('err', err)//判別資料是否正確
      return
    }
...
function deleteBooks(id){
  request.delete(`${apiUrl}/books/${id}`, function (err, response, body) {
    if(err){
      console.log('err', err)//判別資料是否正確
      return
    }
...
function updateBooks(id, name){
  request.patch({
    url: `${apiUrl}/books/${id}`,
    form: {
      name/*這邊參考其他人的解法時, 發現有人會使用 newName, 
      但發現這樣會直接讓那本書後面多一個 newName 的 id, 應該用 name 才會讓原本 id更改*/
    }
  }, (err) => {
    if (err){
